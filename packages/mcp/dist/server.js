...
// ── Tool: adapt_spec ──────────────────────────────────────────────────────────
server.tool('adapt_spec', 'Convert an AgentUXSpec JSON object into a renderable AgentPanelSpec. ' +
    'Use this when building a Skillz agent to auto-generate its panel layout.', {
    uiSpec: zod_1.z.string().describe('AgentUXSpec as a JSON string'),
}, async ({ uiSpec }) => {
    let parsed;
    try {
    ...
    const validation = (0, panel_adapter_core_1.validateUXSpec)(parsed);
    if (!validation.valid) {
        return {
            content: [{
                    type: 'text',
    ...
    const panelSpec = (0, panel_adapter_core_1.adapt)(parsed);
    return {
        content: [{
                type: 'text',
...
server.tool('suggest_sections', 'Given a partially-filled AgentUXSpec, suggest what to fill in next. ' +
    'Provide a description of your agent for better suggestions.', {
    partial: zod_1.z.string().describe('Partial AgentUXSpec as JSON string (can be {})'),
    agentName: zod_1.z.string().optional().describe('Agent name'),
    description: zod_1.z.string().optional().describe('Agent description in plain English'),
    ...
    const suggestion = (0, panel_adapter_core_1.suggest)(parsed, { agentName, description });
    const lines = [
        `Suggested field: **${suggestion.field}**`,
        '',
        ...suggestion.suggestions.map((s, i) => `${i + 1}. [${s.confidence.toUpperCase()}] ${JSON.stringify(s.value)}\n   Reason: ${s.reason}`),
    ];
    return { content: [{ type: 'text', text: lines.join('\n') }] };
});
// ── Tool: validate_spec ───────────────────────────────────────────────────────
server.tool('validate_spec', 'Validate an AgentUXSpec against the Skillz schema. Returns validation errors if any.', {
    uiSpec: zod_1.z.string().describe('AgentUXSpec as a JSON string'),
}, async ({ uiSpec }) => {
    let parsed;
    try {
    ...
    const result = (0, panel_adapter_core_1.validateUXSpec)(parsed);
    if (result.valid) {
