 ...
 */
export type BaseArchitecture = 'calendar-view' | 'table' | 'form-result' | 'dashboard' | 'card-feed' | 'settings-form' | 'chat-augment' | 'flow-wizard';
export type DataSourceType = 'google-calendar' | 'outlook-calendar' | 'ical-feed' | 'todoist' | 'notion' | 'gmail' | 'google-maps' | 'google-places' | 'google-people' | 'sendgrid' | 'web-scraper' | 'gemini-search' | 'supabase-internal' | 'llm-inference' | 'stripe' | 'agent-output';
...
export type DataOutputType = 'calendar-events' | 'task-list' | 'contact-list' | 'metrics-chart' | 'text-summary' | 'notification' | 'email' | 'booking-record' | 'payment-record' | 'scored-list' | 'travel-buffer-event' | 'trend-report' | 'social-plan' | 'supply-chain-record' | 'recommendation-list';
export interface DataOutputDeclaration {
    type: DataOutputType;
    label: string;
    rawContextKey: string;
}
export type EditToolType = 'calendar-card-editor' | 'table-row-editor' | 'form-submit' | 'tag-input' | 'toggle-switch' | 'select-dropdown' | 'drag-reorder' | 'inline-text-edit';
...
export type PanelSection = TextSummarySection | TextListSection | TagInputSection | SelectSection | ToggleSection | TableSection | CardListSection | ActionFormSection | AgentEmbedSection;
