...
const ARCH_KEYWORDS: Record<BaseArchitecture, string[]> = {
...
const ARCH_SOURCES: Record<BaseArchitecture, Array<{ type: DataSourceType; required: boolean }>> = {
...
const ARCH_OUTPUTS: Record<BaseArchitecture, DataOutputType[]> = {
...
const KEYWORD_SOURCES: Array<{ keywords: string[]; source: DataSourceType }> = [
 ...
 */
export function suggest(partial: Partial<AgentUXSpec>, context?: SuggestContext): SuggestionResult {
...
function suggestArchitecture(tokens: string[]): SuggestionResult {
  const scored: Array<{ arch: BaseArchitecture; score: number }> = [];

  for (const [arch, keywords] of Object.entries(ARCH_KEYWORDS) as [BaseArchitecture, string[]][]) {
    const score = countMatches(tokens, keywords);
    if (score > 0) scored.push({ arch, score });
  }
  ...
  const suggestions = scored.slice(0, 3).map(({ arch, score }) => ({
    value: arch,
    confidence: (score >= 2 ? 'high' : 'medium') as 'high' | 'medium' | 'low',
    reason: `Description matches ${score} keyword${score > 1 ? 's' : ''} for "${arch}".`,
  }));

  if (suggestions.length === 0) {
    suggestions.push({ value: 'card-feed' as BaseArchitecture, confidence: 'low' as const, reason: 'Default — no strong keyword match found.' });
  }

  return { field: 'baseArchitecture', suggestions };
}

function suggestDataSources(arch: BaseArchitecture, tokens: string[]): SuggestionResult {
  const baseSources = ARCH_SOURCES[arch] ?? [];
  const extra: DataSourceType[] = [];
...
function suggestDataOutputs(arch: BaseArchitecture, _tokens: string[]): SuggestionResult {
...
function suggestEditTools(arch: BaseArchitecture): SuggestionResult {
  const toolMap: Record<BaseArchitecture, Array<{ type: string; label: string }>> = {
...
function suggestContributions(arch: BaseArchitecture): SuggestionResult {
  // Contribution suggestions — what data does this agent inject INTO another agent's rawContext?
  const contribMap: Record<BaseArchitecture, Array<{ targetAgentId: string; targetKey: string; fromOutputKey: string; strategy: string; label: string; reason: string }>> = {
    'form-result':   [
...
function suggestAgentTools(arch: BaseArchitecture): SuggestionResult {
  // AgentTools suggestions — what other agent's actions does this agent call at runtime?
  const toolMap: Record<BaseArchitecture, Array<{ providerAgentId: string; actionName: string; label: string; reason: string }>> = {
    'form-result':   [
  ...
  const suggestions = tools.map(t => ({
    value: { agentId: t.providerAgentId, actionName: t.actionName, label: t.label, invokeOn: 'on-action' as const },
    confidence: 'medium' as const,
    reason: t.reason,
  }));
